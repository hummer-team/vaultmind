...
window.addEventListener('message', async (event) => {
    ...
    try {
        const safeLog = {
            ...
            hasBuffer: event.data?.buffer instanceof ArrayBuffer,
            fileName: event.data?.fileName,
        };
        console.log('[Sandbox] Received message summary:', safeLog);
    } catch (e) {
        console.warn('[Sandbox] Failed to build safe log summary:', e);
    }
